...
#if 0
static void
send_buffer(char** buf, int* size, int len)
{
  ensure_buffer(buf, size, len + 1);
  (*buf)[len] = '\0';
  write_reply(*buf);
}

static void
send_quote_buffer(char** buf, int* size, int len)
{
  char* obuf;
  char* p, * q, * end;

  obuf = (char*) alloca(len * 2 + 1);
  end = *buf + len;
  for (p = *buf, q = obuf; p < end;) {
    switch (*p) {
    case '\"':
    case '\\':
      *q++ = '\\';
